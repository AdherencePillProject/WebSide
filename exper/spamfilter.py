...
def parse(text_file):
    # This function parses the text_file passed into it into a set of words. Right now it just splits up the file by
    # blank spaces, and returns the set of unique strings used in the file.
...
def makedictionary(spam_directory, ham_directory, dictionary_filename):
    ...
    # These for loops walk through the files and construct the dictionary. The dictionary, words, is constructed
    # so that words[word]['spam'] gives the probability of observing that word, given we have a spam document
    # P(word|spam), and words[word]['ham'] gives the probability of observing that word, given a ham document
    # P(word|ham). Right now, all it does is initialize both probabilities to 0.
...
def is_spam(content, dictionary, spam_prior_probability):
    # TODO: Update this function. Right now, all it does is checks whether the spam_prior_probability is more than
    # half the data. If it is, it says spam for everything. Else, it says ham for everything. You need to update
    # it to make it use the dictionary and the content of the mail. Here is where your naive Bayes classifier goes.
    p_spam = 0.0
    p_ham = 0.0
    for word in content:
        if word in dictionary.keys():
            p_spam += math.log(dictionary[word]['spam'])
            p_ham += math.log(dictionary[word]['ham'])
    p_spam += math.log(spam_prior_probability)
    p_ham += math.log(1 - spam_prior_probability)
    if p_spam > p_ham:
        return True
    elif spam_prior_probability >= 0.5:
        return True
    else:
        return False
...
def spamsort(mail_directory, spam_directory, ham_directory, dictionary, spam_prior_probability):
    ...
    for m in mail:
        content = parse(open(os.path.join(mail_directory, m)))
        spam = is_spam(content, dictionary, spam_prior_probability)
        if spam:
            shutil.copy(os.path.join(mail_directory, m), spam_directory)
        else:
            shutil.copy(os.path.join(mail_directory, m), ham_directory)

if __name__ == "__main__":
    # Here you can test your functions. Pass it a training_spam_directory, a training_ham_directory, and
    # a mail_directory that is filled with unsorted mail on the command line. It will create two directories
    # in the directory where this file exists: sorted_spam, and sorted_ham. The files will show up  in this
    # directories according to the algorithm you developed.
